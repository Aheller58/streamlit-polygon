...
def main():
    ...
    with st.container():
        ...
        with tab1:
            # Add Reset Button at the top with warning
            ...
            with col1:
                num_leads = st.number_input("Number of Leads:", min_value=0, step=1, value=100)
                num_appointments = st.number_input("Number of Appointments:", min_value=0, step=1, value=10)

            with col2:
                num_closings = st.number_input("Number of Closings:", min_value=0, step=1, value=1)
            ...
            if st.session_state.uploaded_data is not None:
                ...
                create_metrics_dashboard(st.session_state.combined_data)
            else:
                st.session_state.combined_data = new_row
                st.write("Current Data Overview:")
                st.dataframe(new_row)
                create_metrics_dashboard(new_row)

            # Update historical data for other tabs
            st.session_state.historical_data = st.session_state.combined_data
